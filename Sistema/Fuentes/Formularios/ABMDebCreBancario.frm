VERSION 5.00
Begin VB.Form ABMDebCreBancario 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Datos de Débitos y Créditos Bancarios..."
   ClientHeight    =   1860
   ClientLeft      =   2700
   ClientTop       =   2625
   ClientWidth     =   5610
   ...
   EndProperty
   Icon            =   "ABMDebCreBancario.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.CommandButton cmdAyuda 
      Height          =   315
      Left            =   195
      Picture         =   "ABMDebCreBancario.frx":000C
      Style           =   1  'Graphical
      TabIndex        =   7
      Top             =   1395
      Visible         =   0   'False
   ...
   End
End
Attribute VB_Name = "ABMDebCreBancario"
...
Const cTabla = "TIPO_DEBCRE_BANCARIO"
Const cCampoID = "TDCB_CODIGO"
Const cDesRegistro = "Débito y Crédito Bancarios"
    ...
    If Validar(vMode) = True Then
        ...
        Select Case vMode
            Case 1 'nuevo
                ...
                cSQL = cSQL & "     (TDCB_CODIGO, TDCB_DESCRI, TDCB_TIPO) "
                cSQL = cSQL & "VALUES "
                cSQL = cSQL & "     (" & txtID.Text & ", " & XS(txtDescri.Text) & ", " & XS(Left(cboTipo.List(cboTipo.ListIndex), 1)) & ") "
            
            Case 2 'editar
                
                cSQL = "UPDATE " & cTabla & " SET "
                cSQL = cSQL & "     TDCB_DESCRI = " & XS(txtDescri.Text)
                cSQL = cSQL & "     ,TDCB_TIPO = " & XS(Left(cboTipo.List(cboTipo.ListIndex), 1))
                cSQL = cSQL & " WHERE TDCB_CODIGO  = " & txtID.Text
            
            Case 4 'eliminar
            
                cSQL = "DELETE FROM " & cTabla & " WHERE TDCB_CODIGO  = " & txtID.Text
...
Private Sub Form_Load()
    ...
    If vMode <> 1 Then
        If vFieldID <> "0" Then
            cSQL = "SELECT * FROM " & cTabla & "  WHERE TDCB_CODIGO = " & Mid(vFieldID, 2, Len(vFieldID) - 2)
            rec.Open cSQL, DBConn, adOpenStatic, adLockOptimistic
            If (rec.BOF And rec.EOF) = 0 Then
                'si encontró el registro muestro los datos
                txtID.Text = rec!TDCB_CODIGO
                txtDescri.Text = rec!TDCB_DESCRI
                If Trim(rec!TDCB_TIPO) = "C" Then
                    cboTipo.ListIndex = 0
                Else
                    cboTipo.ListIndex = 1
                End If
