VERSION 5.00
Begin VB.Form ABMEstadoCheque 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Datos de Estado Cheques..."
   ClientHeight    =   2025
   ClientLeft      =   2700
   ClientTop       =   2625
   ClientWidth     =   4530
   ...
   Begin VB.CommandButton cmdAyuda 
      Height          =   315
      Left            =   210
      Picture         =   "ABMEstadoCheque.frx":000C
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   1605
      Width           =   330
   ...
   End
End
Attribute VB_Name = "ABMEstadoCheque"
...
Const cTabla = "ESTADO_CHEQUE"
Const cCampoID = "ECH_CODIGO"
Const cDesRegistro = "Estado Cheques"
...
Function SetMode(pMode As Integer)
    ...
    Select Case pMode
        Case 1
            ...
            Me.Caption = "Nuevo Estado Cheque..."
            AcCtrl txtID
        Case 2
            cmdAceptar.Enabled = False
            Me.Caption = "Editando Estado Cheque..."
            DesacCtrl txtID
        Case 3
            cmdAceptar.Visible = False
            Me.Caption = "Datos de Estado Cheque..."
            DesacCtrl txtID
        Case 4
            cmdAceptar.Enabled = True
            Me.Caption = "Eliminando Estado Cheque..."
            DesacCtrl txtID
    End Select
    ...
    If pMode <> 4 Then
        ...
        If txtID.Text = "" Then
            ...
            MsgBox "Falta información." & Chr(13) & _
                             "Ingrese la Identificación del Estado de Cheque antes de aceptar.", vbCritical + vbOKOnly, App.Title
            txtID.SetFocus
            Exit Function
        ElseIf txtDescri.Text = "" Then
            Beep
            MsgBox "Falta información." & Chr(13) & _
                             "Ingrese la descripción del Estado de Cheque antes de aceptar.", vbCritical + vbOKOnly, App.Title
            txtDescri.SetFocus
            Exit Function
        End If
    End If
    ...
    If Validar(vMode) = True Then
        ...
        Select Case vMode
            Case 1 'nuevo
                ...
                cSQL = cSQL & "     (ECH_CODIGO, ECH_DESCRI) "
                cSQL = cSQL & "VALUES "
                cSQL = cSQL & "     (" & txtID.Text & ", " & XS(txtDescri.Text, True) & ") "
            
            Case 2 'editar
                
                cSQL = "UPDATE " & cTabla & " SET "
                cSQL = cSQL & "     ECH_DESCRI = " & XS(txtDescri.Text, True)
                cSQL = cSQL & " WHERE ECH_CODIGO  = " & txtID.Text
            
            Case 4 'eliminar
            
                cSQL = "DELETE FROM " & cTabla & " WHERE ECH_CODIGO  = " & txtID.Text
...
Private Sub Form_Load()
    ...
    If vMode <> 1 Then
        If vFieldID <> "0" Then
            cSQL = "SELECT * FROM " & cTabla & "  WHERE ECH_CODIGO = " & Mid(vFieldID, 2, Len(vFieldID) - 2)
            rec.Open cSQL, DBConn, adOpenStatic, adLockOptimistic
            If (rec.BOF And rec.EOF) = 0 Then
                'si encontró el registro muestro los datos
                txtID.Text = rec!ECH_CODIGO
                txtDescri.Text = rec!ECH_DESCRI
            Else
                Beep
