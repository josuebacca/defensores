VERSION 5.00
Begin VB.Form ABMTipoGastosBancarios 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Datos de Gastos Bancarios..."
   ClientHeight    =   2025
   ClientLeft      =   2700
   ClientTop       =   2625
   ClientWidth     =   4530
   ...
   EndProperty
   Icon            =   "ABMTipoGastosBancarios.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.CommandButton cmdAyuda 
      Height          =   315
      Left            =   210
      Picture         =   "ABMTipoGastosBancarios.frx":000C
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   1605
      Visible         =   0   'False
   ...
   End
End
Attribute VB_Name = "ABMTipoGastosBancarios"
...
Const cTabla = "TIPO_GASTO_BANCARIO"
Const cCampoID = "TGB_CODIGO"
Const cDesRegistro = "Gastos Bancarios"
...
Function SetMode(pMode As Integer)
    ...
    Select Case pMode
        Case 1
            ...
            Me.Caption = "Nuevo Gasto Bancario..."
            AcCtrl txtID
        Case 2
            cmdAceptar.Enabled = False
            Me.Caption = "Editando Gasto Bancario..."
            DesacCtrl txtID
        Case 3
            cmdAceptar.Visible = False
            Me.Caption = "Datos Gastos Bancarios..."
            DesacCtrl txtID
        Case 4
            cmdAceptar.Enabled = True
            Me.Caption = "Eliminando Gasto Bancario..."
            DesacCtrl txtID
    End Select
    ...
    If pMode <> 4 Then
        ...
        If txtID.Text = "" Then
            ...
            MsgBox "Falta información." & Chr(13) & _
                             "Ingrese la Identificación del Gasto Bancario antes de aceptar.", vbCritical + vbOKOnly, App.Title
            txtID.SetFocus
            Exit Function
        ElseIf txtDescri.Text = "" Then
            Beep
            MsgBox "Falta información." & Chr(13) & _
                             "Ingrese la descripción del Gasto Bancario antes de aceptar.", vbCritical + vbOKOnly, App.Title
            txtDescri.SetFocus
            Exit Function
        End If
    End If
    ...
    If Validar(vMode) = True Then
        ...
        Select Case vMode
            Case 1 'nuevo
                ...
                cSQL = cSQL & "     (TGB_CODIGO, TGB_DESCRI) "
                cSQL = cSQL & "VALUES "
                cSQL = cSQL & "     (" & txtID.Text & ", " & XS(txtDescri.Text) & ") "
            
            Case 2 'editar
                
                cSQL = "UPDATE " & cTabla & " SET "
                cSQL = cSQL & "     TGB_DESCRI = " & XS(txtDescri.Text)
                cSQL = cSQL & " WHERE TGB_CODIGO  = " & txtID.Text
            
            Case 4 'eliminar
            
                cSQL = "DELETE FROM " & cTabla & " WHERE TGB_CODIGO  = " & txtID.Text
...
Private Sub Form_Load()
    ...
    If vMode <> 1 Then
        If vFieldID <> "0" Then
            cSQL = "SELECT * FROM " & cTabla & "  WHERE TGB_CODIGO = " & Mid(vFieldID, 2, Len(vFieldID) - 2)
            rec.Open cSQL, DBConn, adOpenStatic, adLockOptimistic
            If (rec.BOF And rec.EOF) = 0 Then
                'si encontró el registro muestro los datos
                txtID.Text = rec!TGB_CODIGO
                txtDescri.Text = rec!TGB_DESCRI
            Else
                Beep
