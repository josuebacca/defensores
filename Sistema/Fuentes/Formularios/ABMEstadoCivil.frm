VERSION 5.00
Begin VB.Form ABMEstadoCivil 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Datos del Estado Civil..."
   ClientHeight    =   2025
   ClientLeft      =   2700
   ClientTop       =   2625
   ClientWidth     =   4530
   ...
   Begin VB.CommandButton cmdAyuda 
      Height          =   315
      Left            =   210
      Picture         =   "ABMEstadoCivil.frx":000C
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   1605
      Width           =   330
   ...
   End
End
Attribute VB_Name = "ABMEstadoCivil"
...
Const cTabla = "ESTADO_CIVIL"
Const cCampoID = "EST_CODIGO"
Const cDesRegistro = "Estado Civil"
...
Function SetMode(pMode As Integer)
    ...
    Select Case pMode
        Case 1
            ...
            Me.Caption = "Nuevo Estado Civil..."
            txtID_LostFocus
            DesacCtrl txtID
        Case 2
            cmdAceptar.Enabled = False
            Me.Caption = "Editando Estado Civil..."
            DesacCtrl txtID
        Case 3
            cmdAceptar.Visible = False
            Me.Caption = "Datos del Estado Civil..."
            DesacCtrl txtID
        Case 4
            cmdAceptar.Enabled = True
            Me.Caption = "Eliminando Estado Civil..."
            DesacCtrl txtID
    End Select
    ...
    If pMode <> 4 Then
        ...
        If txtID.Text = "" Then
            ...
            MsgBox "Falta información." & Chr(13) & _
                             "Ingrese la Identificación del Estado Civil antes de aceptar.", vbCritical + vbOKOnly, App.Title
            txtID.SetFocus
            Exit Function
        ElseIf txtDescri.Text = "" Then
            Beep
            MsgBox "Falta información." & Chr(13) & _
                             "Ingrese la descripción del Estado Civil antes de aceptar.", vbCritical + vbOKOnly, App.Title
            txtDescri.SetFocus
            Exit Function
        End If
    End If
    ...
    If Validar(vMode) = True Then
        ...
        Select Case vMode
            Case 1 'nuevo
                ...
                cSQL = cSQL & "     (EST_CODIGO, EST_DESCRI) "
                cSQL = cSQL & "VALUES "
            ...
            Case 2 'editar
                ...
                cSQL = cSQL & "     EST_DESCRI = " & XS(txtDescri.Text, True)
                cSQL = cSQL & " WHERE EST_CODIGO  = " & XN(txtID.Text)
            
            Case 4 'eliminar
            
                cSQL = "DELETE FROM " & cTabla & " WHERE EST_CODIGO  = " & XN(txtID.Text)
...
Private Sub Form_Load()
    ...
    If vMode <> 1 Then
        If vFieldID <> "0" Then
            cSQL = "SELECT * FROM " & cTabla & "  WHERE EST_CODIGO = " & Mid(vFieldID, 2, Len(vFieldID) - 2)
            rec.Open cSQL, DBConn, adOpenStatic, adLockOptimistic
            If (rec.BOF And rec.EOF) = 0 Then
                'si encontró el registro muestro los datos
                txtID.Text = rec!EST_CODIGO
                txtDescri.Text = Trim(ChkNull(rec!EST_DESCRI))
            Else
                Beep
