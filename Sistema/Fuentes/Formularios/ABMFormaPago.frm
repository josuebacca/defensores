...
Const cTabla = "FORMA_PAGO"
Const cCampoID = "FPG_CODIGO"
Const cDesRegistro = "Forma de Pago"
    ...
    If Validar(vMode) = True Then
        ...
        Select Case vMode
            Case 1 'nuevo
                ...
                cSQL = cSQL & "     (FPG_CODIGO, FPG_DESCRI) "
                cSQL = cSQL & "VALUES "
            ...
            Case 2 'editar
                ...
                cSQL = cSQL & "     FPG_DESCRI = " & XS(txtDescri.Text)
                cSQL = cSQL & " WHERE FPG_CODIGO  = " & XN(txtID.Text)
            
            Case 4 'eliminar
            
                cSQL = "DELETE FROM " & cTabla & " WHERE FPG_CODIGO  = " & XN(txtID.Text)
...
Private Sub Form_Load()
    ...
    If vMode <> 1 Then
        If vFieldID <> "0" Then
            cSQL = "SELECT * FROM " & cTabla & "  WHERE FPG_CODIGO = " & Mid(vFieldID, 2, Len(vFieldID) - 2)
            rec.Open cSQL, DBConn, adOpenStatic, adLockOptimistic
            If (rec.BOF And rec.EOF) = 0 Then
                'si encontró el registro muestro los datos
                txtID.Text = rec!FPG_CODIGO
                txtDescri.Text = Trim(ChkNull(rec!FPG_DESCRI))
            Else
                Beep
