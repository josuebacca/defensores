VERSION 5.00
Begin VB.Form ABMSexo 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Datos del Sexo..."
   ClientHeight    =   2025
   ClientLeft      =   2700
   ClientTop       =   2625
   ClientWidth     =   4530
...
Const cTabla = "SEXO"
Const cCampoID = "SEX_CODIGO"
Const cDesRegistro = "Sexo"
...
Function SetMode(pMode As Integer)
    ...
    Select Case pMode
        Case 1
            ...
            Me.Caption = "Nuevo Sexo..."
            txtID_LostFocus
            DesacCtrl txtID
        Case 2
            cmdAceptar.Enabled = False
            Me.Caption = "Editando Sexo..."
            DesacCtrl txtID
        Case 3
            cmdAceptar.Visible = False
            Me.Caption = "Datos del Sexo..."
            DesacCtrl txtID
        Case 4
            cmdAceptar.Enabled = True
            Me.Caption = "Eliminando Sexo..."
            DesacCtrl txtID
    End Select
    ...
    If pMode <> 4 Then
        ...
        If txtID.Text = "" Then
            ...
            MsgBox "Falta información." & Chr(13) & _
                             "Ingrese la Identificación del Sexo antes de aceptar.", vbCritical + vbOKOnly, App.Title
            txtID.SetFocus
            Exit Function
        ElseIf txtDescri.Text = "" Then
            Beep
            MsgBox "Falta información." & Chr(13) & _
                             "Ingrese la descripción del Sexo antes de aceptar.", vbCritical + vbOKOnly, App.Title
            txtDescri.SetFocus
            Exit Function
        End If
    End If
    ...
    If Validar(vMode) = True Then
        ...
        Select Case vMode
            Case 1 'nuevo
                ...
                cSQL = cSQL & "     (SEX_CODIGO, SEX_DESCRI) "
                cSQL = cSQL & "VALUES "
            ...
            Case 2 'editar
                ...
                cSQL = cSQL & "     SEX_DESCRI = " & XS(txtDescri.Text, True)
                cSQL = cSQL & " WHERE SEX_CODIGO  = " & XN(txtID.Text)
            
            Case 4 'eliminar
            
                cSQL = "DELETE FROM " & cTabla & " WHERE SEX_CODIGO  = " & XN(txtID.Text)
...
Private Sub Form_Load()
    ...
    If vMode <> 1 Then
        If vFieldID <> "0" Then
            cSQL = "SELECT * FROM " & cTabla & "  WHERE SEX_CODIGO = " & Mid(vFieldID, 2, Len(vFieldID) - 2)
            rec.Open cSQL, DBConn, adOpenStatic, adLockOptimistic
            If (rec.BOF And rec.EOF) = 0 Then
                'si encontró el registro muestro los datos
                txtID.Text = rec!SEX_CODIGO
                txtDescri.Text = Trim(ChkNull(rec!SEX_DESCRI))
            Else
                Beep
