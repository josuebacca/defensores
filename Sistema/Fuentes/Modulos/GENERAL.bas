...
Global Const VRepresentada  As String * 1 = "0"
'Nombre de Usuario
...
'PARA SABER QUE FORMULARIO LA LLAMO
Global mQuienLlamo As String
Global mMeDioError As Boolean
...
'para evitar que se apague el Teclado Numerico
'cumple la funcion del sendkeys tab
'GRACIAS A ELIANA GIACOMINI - GOMEZ
...
Public Function EscribirCelda(KeyAscii As Integer, grid As MSFlexGrid)
    If grid.row > 0 Then
       If KeyAscii = 8 Then
          If Len(grid.TextMatrix(grid.row, grid.Col)) = 0 Then
             Exit Function
          End If
          grid.TextMatrix(grid.row, grid.Col) = Left(grid.TextMatrix(grid.row, grid.Col), Len(grid.TextMatrix(grid.row, grid.Col)) - 1)
          Exit Function
       End If
       If KeyAscii = 13 And grid.Rows > grid.row + 1 Then
          grid.row = grid.row + 1
...
Public Sub CambiaColorAFilaDeGrilla(G As MSFlexGrid, Fila As Integer, Optional ColorFuente As ColorConstants, Optional ColorFondo As ColorConstants, Optional mBold As Boolean)
    Dim J As Integer
...
Public Sub Set_Impresora()
    ...
    Printer.ScaleMode = 7
    'Printer.Height = 12250 '12190 '8642
    'Printer.Width = 12077 '12220
    '-----------------------------------------------------------------
...
Public Sub CambioTipoLetraControles(Formulario As Form, Letrita As String)
   ' Así lo llamo:
   ' CambioTipoLetraControles Me, "Tahoma"
   
   ' Si usas la Función Cargar alcanza con agregar:
   '   CambioTipoLetraControles formu, "Tahoma"
   '      una sola vez y listo!!!! UNA MAGIA
   
   'Cambia el Tipo de Letra de Todos los OBJETOS del Formulario
   For Each Control In Formulario.Controls 'Revisar los controles del formulario
        If TypeOf Control Is TextBox _
        Or TypeOf Control Is ListBox _
        Or TypeOf Control Is MSFlexGrid _
        Or TypeOf Control Is Label _
        Or TypeOf Control Is Frame _
        Or TypeOf Control Is CommandButton _
        Or TypeOf Control Is SSTab _
        Or TypeOf Control Is Form _
        Or TypeOf Control Is MDIForm _
        Or TypeOf Control Is OptionButton _
        Or TypeOf Control Is CheckBox _
        Or TypeOf Control Is MaskEdBox _
        Or TypeOf Control Is ComboBox Then
           Control.Font = Letrita
        End If
   Next Control
End Sub

Public Sub CambioFormatoGrilla(Formulario As Form)
   ' Así lo llamo:
   ' CambioColorGrilla Me
   
   ' Si usas la Función Cargar alcanza con agregar:
   '    CambioColorGrilla Me
   '      una sola vez y listo!!!! UNA MAGIA
   
   'Cambia el Color de SELECCION de las Grillas
   For Each Control In Formulario.Controls 'Revisar los controles del formulario
        If TypeOf Control Is MSFlexGrid Then
           Control.BackColorSel = &HFFFF80   'Celeste Fondo de Seleccion de Filas
           Control.ForeColorSel = &H80000008 'Negro   Letra de Seleccion de Filas
           Control.HighLight = 2             'Resalta las celdas seleccionadas cuando el control tiene el enfoque.
                      
           Control.Rows = 1
           Control.BorderStyle = flexBorderNone
           Control.row = 0
           For i = 0 To Control.Cols - 1
               Control.Col = i
               Control.CellForeColor = &HFFFFFF 'FUENTE COLOR BLANCO
               Control.CellBackColor = &H808080 'GRIS OSCURO
               Control.CellFontBold = True
           Next
           
           Control.BackColorBkg = &H8000000F   'Color del Fondo de la Grilla
        End If
   Next Control
End Sub
